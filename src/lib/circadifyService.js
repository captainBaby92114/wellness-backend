...
async function uploadBuffer(uploadUrl, buffer, mimeType) {
  ...
  if (!res.ok) throw new Error(`uploadBuffer failed: ${res.status}`);
}
...
async function notifyComplete(sessionId) {
  ...
  if (!res.ok) throw new Error(`notifyComplete failed: ${res.status}`);
}

async function pollResult(sessionId, {maxAttempts = 10, intervalMs = 3000} = {}) {
  for (let i = 0; i < maxAttempts; i++) {
    ...
    if (data.status === 'completed') return data.metrics;
...
async function getMetrics(fileBuffer, mimeType) {
  try {
    ...
    console.error('[Circadify] error:', err.message);
    return null;
  }
}
